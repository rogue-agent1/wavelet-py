...
def haar_forward(x):
    n=len(x);assert n&(n-1)==0
    a=list(x);h=n
    while h>1:
        h//=2;tmp=list(a)
        for i in range(h):
            a[i]=(tmp[2*i]+tmp[2*i+1])/math.sqrt(2)
            a[h+i]=(tmp[2*i]-tmp[2*i+1])/math.sqrt(2)
    return a
...
def main():
    if len(sys.argv)>1 and sys.argv[1]=="--test":
        x=[1,2,3,4,5,6,7,8]
        c=haar_forward(x);r=haar_inverse(c)
        assert all(abs(a-b)<1e-10 for a,b in zip(x,r)),f"Roundtrip failed: {r}"
